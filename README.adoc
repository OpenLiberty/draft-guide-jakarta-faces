...
In your dev mode console, type `r` and press `enter/return` key to restart the Liberty instance so that Liberty reads the configuration changes. When you see the following message, your Liberty instance is ready in dev mode:
...
To format the table, set the `styleClass`, `headerClass`, and `rowClasses` attributes in the [hotspot=dataBind file=0]`<h:dataTable>` tag, where the style elements are defined in the [hotspot=systemLoadsTableClasses file=2]`src/main/webapp/resources/css/styles.css` file.
